...
//////////////////////////////////////////////////////////////////////////////
// These functions relate IO events with bytes to those with PBMessages
// 

function AstractifyNetEventToPBIo(evt:NetEvent) : PBIo
{
    match evt
...
function {:opaque} AbstractifyRawLogToIos(rawlog:seq<NetEvent>) : seq<PBIo>
    ensures |AbstractifyRawLogToIos(rawlog)| == |rawlog|;
    ensures forall i {:trigger AstractifyNetEventToPBIo(rawlog[i])} 
                     {:trigger AbstractifyRawLogToIos(rawlog)[i]} :: 
                0 <= i < |rawlog| ==> AbstractifyRawLogToIos(rawlog)[i] == AstractifyNetEventToPBIo(rawlog[i]);
{
    if (rawlog==[]) then [] else [AstractifyNetEventToPBIo(rawlog[0])] + AbstractifyRawLogToIos(rawlog[1..])
}

lemma lemma_EstablishAbstractifyRawLogToIos(rawlog:seq<NetEvent>, ios:seq<PBIo>)
    requires |rawlog| == |ios|;
    requires forall i :: 0<=i<|rawlog| ==> ios[i] == AstractifyNetEventToPBIo(rawlog[i]);
...
datatype ReceiveResult = RRFail() | RRTimeout() | RRPacket(cpacket:CPBPacket)
...
method Receive(netClient:NetClient, localAddr:EndPoint) 
    ...
    ensures rr.RRPacket? ==>
        ...
        && AbstractifyCPBPacket(rr.cpacket) == AbstractifyNetPacket(netEvent.r)
...
predicate SendLogEntryReflectsPacket(event:NetEvent, cpacket:CPBPacket)
{
       event.LIoOpSend?
    && AbstractifyCPBPacket(cpacket) == AbstractifyNetPacket(event.s)
}

predicate SendLogReflectsPacket(netEventLog:seq<NetEvent>, packet:Option<CPBPacket>)
{
    match packet {
...
method SendPacket(netClient:NetClient, opt_packet:Option<CPBPacket>, ghost localAddr:EndPoint) returns (ok:bool, ghost netEventLog:seq<NetEvent>)
    requires NetClientIsValid(netClient);
    requires EndPoint(netClient.MyPublicKey()) == localAddr;
    requires OptionCPBPacketValid(opt_packet);
                  ...
                  && SendLogReflectsPacket(netEventLog, opt_packet));
{
    ...
    if opt_packet.None? {
        ...
        var buffer := MarshallPBMessage(cpacket.msg);
