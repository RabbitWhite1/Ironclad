            ...
            && forall i :: 0 <= i < |net_event_log| - 1 ==> net_event_log[i].LIoOpReceive? || net_event_log[i+1].LIoOpSend?
{
  ...
  var old_commit_index := server_impl.state.commit_index;
  var old_last_applied := server_impl.state.last_applied;

  if server_impl.state.last_applied >= server_impl.state.commit_index {
    // nothing to apply
    ok := true;
    return;
  }
  ...
  assert server_impl.state.last_applied < server_impl.state.commit_index;
  assert old_last_applied < server_impl.state.commit_index;
  var i := server_impl.state.last_applied + 1;
  if i > |server_impl.state.log| as uint64 - 1 {
    // nothing to apply
    ok := true;
    return;
  }
  var log_entry := server_impl.state.log[i];
  var leader_id;
  ...
  var app_reply := server_impl.state.app_state.HandleRequest(log_entry.req);
  var msg := CMessage_Reply(log_entry.seqno, 1, leader_id, app_reply);
  var packet := CPacket(log_entry.client_ep, server_impl.GetMyEp(), msg);
  var outbound_packets := OutboundPacket(Some(packet));
  assert server_impl.state.last_applied < |server_impl.state.log| as uint64;

  server_impl.state := server_impl.state.(last_applied := server_impl.state.last_applied  + 1);
