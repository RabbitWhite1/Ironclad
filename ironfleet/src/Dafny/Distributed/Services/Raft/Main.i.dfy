...
include "RaftDistributedSystem.i.dfy"
...
import opened AS_s = AbstractServiceRaft_s
import opened DS_s = Raft_DistributedSystem_i
...
//   requires |db| > 0
//   ensures |sb| == |db|;
//   ensures Service_Init(sb[0], MapSeqToSet(config.server_eps, x=>x));
...
// {
//   if |db| == 1 {
//     sb := [ServiceState'(MapSeqToSet(config.server_eps, x=>x), {}, {})];
//   } else {
//     var rest := RefinementToServiceState(config, all_but_last(db));
//     var last := last(db);
//     sb := rest + [ServiceState'(MapSeqToSet(config.server_eps, x=>x), {}, {})];
//   }
// }
